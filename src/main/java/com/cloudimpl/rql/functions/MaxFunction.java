 ...
 * @author nuwansa
 */
public class MaxFunction extends AggregateColumnNode {
    ...
    public void  eval(List<JsonObject> input, JsonObject output) {
        ...
        if(dataType == 1)
        {
            double d = input.stream().filter(json -> json.keySet().contains(columnName)).map(json -> json.get(columnName))
                .map(el->toVal(el)).mapToDouble(BigDecimal::doubleValue).max().getAsDouble();
        ...
        else
        {
            String max = input.stream().filter(json -> json.keySet().contains(columnName)).map(json -> json.get(columnName))
                    .map(el->el.getAsString()).max((l,r)->l.compareTo(r)).get();
